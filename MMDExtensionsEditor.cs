    ...
    public class MMDExtensionsEditor : Editor
    {
        ...
        public static void CreateMorphAnimation()
        {
            ...
            if (Path.GetExtension(path).ToUpper().Contains("VMD"))
            {
                ...
                var chara = GameObject.Find("Face");
                ...
                //MMD的表情作为键，取出对应的vroid表情的blendshape值
                Dictionary<string, string> morphToBS = new Dictionary<string, string>();
                morphToBS.Add("あ", "Fcl_MTH_A");
                morphToBS.Add("い", "Fcl_MTH_I");
                morphToBS.Add("う", "Fcl_MTH_U");
                morphToBS.Add("お", "Fcl_MTH_O");
                morphToBS.Add("えー", "Fcl_MTH_E");
                morphToBS.Add("まばたき", "Fcl_EYE_Close");
                morphToBS.Add("笑い", "Fcl_EYE_Joy");
                morphToBS.Add("真面目", "Fcl_BRW_Angry");
                morphToBS.Add("困る", "browInnerUp");
                morphToBS.Add("にこり", "Fcl_BRW_Joy");
                morphToBS.Add("怒り", "Fcl_BRW_Angry");
                morphToBS.Add("上", "Fcl_BRW_Fun");
                morphToBS.Add("下", "Fcl_ALL_Sorrow");
                morphToBS.Add("ウィンク", "Fcl_EYE_Joy_R");
                morphToBS.Add("ウィンク２", "Fcl_EYE_Close_R");
                morphToBS.Add("ウィンク右", "Fcl_EYE_Joy_L");
                morphToBS.Add("ｳｨﾝｸ２右", "Fcl_EYE_Close_L");
                //morphToBS.Add("はぅ", "");>_<
                //morphToBS.Add("なごみ", "Fcl_EYE_Sorrow");-_-
                morphToBS.Add("びっくり", "Fcl_EYE_Spread");
                morphToBS.Add("瞳小", "Fcl_EYE_Surprised");
                morphToBS.Add("ぺろっ", "tongueOut"); //吐舌
                morphToBS.Add("にやり", "Fcl_MTH_Joy"); //微笑

                foreach (var bs in blendShapeNames)
                {
                    //Debug.Log(bs);
                }
                foreach (var package in keyframes)
                {
                    var name = package.Key;//MMD表情名称

                    var curve = new AnimationCurve(package.ToArray());

                    try
                    {
                        var registerName = blendShapeNames.Where(x => x == morphToBS[name]).First();//当模型的bs与当前帧MMD表情名相对应时
                        //animationClip.SetCurve($"{parentName}/{gameObjectName}", typeof(SkinnedMeshRenderer), $"blendShape.{registerName}", curve);
                        animationClip.SetCurve($"{gameObjectName}", typeof(SkinnedMeshRenderer), $"blendShape.{registerName}", curve);
                    }
                    catch
                    {
                        continue;
